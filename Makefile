...
.PHONY: test
test: manifests generate fmt vet install-envtest ## Run tests.
...
.PHONY: lint
lint: install-golangci-lint ## Run golangci-lint linter & yamllint
	$(GOLANGCI_LINT) run
...
build: manifests generate fmt vet ## Build controller binary.
	go build -o bin/controller github.com/fastforgeinc/tensegrity/cmd/controller

.PHONY: run
run: manifests generate fmt vet ## Run a controller from your host.
	go run github.com/fastforgeinc/tensegrity/cmd/controller
...
build-installers: build-installer build-installer-monitoring build-installer-cert-manager build-installer-monitoring-cert-manager
...
build-installer: manifests generate install-kustomize install-yq ## Generate a consolidated YAML with CRDs and deployment.
	mkdir -p dist
	cd manifests/controller && $(KUSTOMIZE) edit set image controller=${IMG}
...
install-controller-gen: $(CONTROLLER_GEN) ## Download controller-gen locally if necessary.
$(CONTROLLER_GEN): $(LOCALBIN)
	$(call go-install-tool,$(CONTROLLER_GEN),sigs.k8s.io/controller-tools/cmd/controller-gen,$(CONTROLLER_TOOLS_VERSION))

.PHONY: install-envtest
install-envtest: $(ENVTEST) ## Download setup-envtest locally if necessary.
$(ENVTEST): $(LOCALBIN)
